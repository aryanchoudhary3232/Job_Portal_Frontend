  ...
  return (
    ...
    >
      <div>
        ...
        <h2 className="mt-2 text-3xl font-extrabold tracking-tight text-[var(--on-surface)] font-display">Create your student profile</h2>
      </div>
      ...
      <Field label="Full name" name="fullName" placeholder="What is your name?" />
      <Field label="Email ID" name="email" placeholder="Tell us your Email ID" type="email" />
      <Field label="Password" name="password" placeholder="Minimum 6 characters" type="password" />
      <Field label="Mobile number" name="phone" placeholder="98765 43210" />
...
function Field({ label, type = "text", helperText, ...props }: { label: string; name: string; placeholder: string; type?: string; helperText?: string }) {
  return (
    <label className="block">
      ...
      {helperText ? <span className="mt-2 block text-xs font-medium text-[var(--on-surface-variant)]">{helperText}</span> : null}
    </label>
  );
}
