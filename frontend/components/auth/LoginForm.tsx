...
export function LoginForm() {
  ...
  return (
    <form
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          setError(getErrorMessage(response, "Login failed"));
        } finally {
          setBusy(false);
        }
      }}
    >
      ...
      <div className="space-y-4 rounded-2xl border border-[var(--outline-variant)] bg-white/80 p-4">
        <div className="flex items-center justify-between">
          <p className="text-[11px] font-bold uppercase tracking-[0.24em] text-[var(--on-surface-variant)]">Mobile verification</p>
          {otpVerified ? (
            <span className="rounded-full bg-green-100 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-green-700">Verified</span>
          ) : (
            <span className="rounded-full bg-amber-50 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-amber-700">Step 1</span>
          )}
        </div>
        ...
        <label className="block">
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            <input
              ...
              className="h-12 w-full flex-1 rounded-xl bg-[var(--surface-container-low)] px-4 text-sm text-[var(--on-surface)] outline-none transition focus:ring-2 focus:ring-[var(--primary)]"
            ...
            {!otpVerified && (
              <button
                ...
                disabled={otpBusy}
                className="h-12 w-full rounded-xl border border-[var(--outline-variant)] px-4 text-sm font-bold text-[var(--primary)] transition hover:-translate-y-0.5 sm:w-auto"
              >
          ...
          <label className="block">
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
              ...
              <button
                ...
                disabled={otpBusy}
                className="h-12 w-full rounded-xl bg-[var(--primary)] px-4 text-sm font-bold text-white transition hover:-translate-y-0.5 sm:w-auto"
              >
        ...
        {otpError ? (
          <p className={`text-xs font-semibold ${otpError.includes("[Dev Mode]") ? "text-amber-600" : "text-red-600"}`}>{otpError}</p>
        ) : null}
        {otpVerified ? (
          <p className="text-xs font-semibold text-green-700">Mobile verified: +91 {phone}</p>
        ) : null}
      </div>
      ...
      {otpVerified ? (
        ...
        <p className="text-xs font-semibold text-[var(--on-surface-variant)]">
          Verify your mobile number to continue with student sign in.
        </p>
      )}
