        ...
        {!loading && !error ? (
          <>
            <StatGrid items={[
              ...
              { label: "Pipeline", value: applications.length },
              { label: "Interviews", value: applications.filter((item) => item.stage === "INTERVIEW").length },
              ...
              <Panel title="Recent applications" subtitle="Latest candidate movement across your roles.">
                <DataTable
                  ...
                  columns={[
                    ...
                    { key: "stage", label: "Stage", render: (row) => <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold">{row.stage}</span> },
                  ]}
                />
              </Panel>
              <Panel title="Hiring priorities" subtitle="Fast reminders for keeping velocity high.">
                <div className="space-y-3 text-sm text-slate-600">
                  {["Review new applications daily", "Move top candidates to screening within 48h", "Keep job descriptions outcome-focused"].map((item) => (
